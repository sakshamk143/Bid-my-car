...
function initApp() {
  ...
  auctionStore.subscribe((store, changeType) => {
    if (changeType === 'TICK') {
    ...
    } else if (changeType === 'BID_PLACED' || changeType === 'OUTBID' || changeType === 'LIVE_LOT_CHANGE') {
      renderLiveAuctionFloor();
      renderInventoryGrid();
      renderNavbar();
    } else if (changeType === 'FILTER_CHANGE' || changeType === 'WATCHLIST_CHANGE') {
      renderInventoryGrid();
      renderNavbar();
      ...
      renderInstitutionalDesk();
      renderInventoryGrid();
    } else if (changeType === 'MODAL_OPEN' || changeType === 'MODAL_CLOSE') {
      renderVehicleModal();
  ...
  window.filterByWatchlist = () => {
    if (auctionStore.watchlist.size === 0) {
      alert('Your watchlist is empty. Click the bookmark icon on any vehicle card to save it.');
      return;
    }
    window.scrollToSection('inventory-section');
  };
...
function showRoleToast(role) {
  ...
  toast.className = 'fixed bottom-6 right-6 z-50 bg-[#0B2235] border border-[#087CFF] text-[#F3F6F9] text-xs font-semibold px-4 py-3 rounded-2xl shadow-2xl flex items-center gap-2 animate-in slide-in-from-bottom-3 duration-200';
  toast.innerHTML = `
    <span class="w-2 h-2 rounded-full bg-[#087CFF]"></span>
    <span>Role View: <strong class="text-white">${names[role]}</strong></span>
  `;
